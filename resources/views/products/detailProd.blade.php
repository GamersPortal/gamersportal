...
@section('panel_left_content')
    ...
    <div class="row hidden-xs">
        <div class="col-md-12">
            ...
            @if ($product->type=='key')
                <button type="button" ng-controller="ModalCtrl" ng-init="data={'data':{{ $product->id }}}"
                        ng-click="modalOpen({templateUrl:'/modalAllKeys',controller:'getKeysVirtualProducts',resolve:'data'})"
    ...
    <div class="row">
        ...
        <div class="col-md-6" ng-controller="ProductsGallery">
            <img src="img/no-image.jpg" lazy-img='[[getPortrait()]]'
                 ng-init="setPortrait('{{ $product->features['images'][0] }}?w=450')"
        ...
        <div class="col-md-3">
            ...
            <ul class="list-unstyled">
                <li><label class="black_color">{{ trans('store.condition') }}
                        :</label>&nbsp;{{ ucwords($product->condition) }}</li>
                <li><label class="black_color">{{ trans('globals.brand') }}:</label>&nbsp;{{ ucwords($product->brand) }}
                </li>
                @foreach ($product->features as $key => $feature)
                    @if ($key != 'images')
                        <li><label class="black_color">{{ ucwords($key) }}
                                :</label>&nbsp;{{  ucwords( is_array($feature) ? implode(' ', $feature) : $feature ) }}
                        </li>
                    @endif
                @endforeach
                <li>
            ...
            <div class="panel panel-default">
                <div class="panel-body">
                    ...
                    <div class="row">
                        <div class="col-lg-12">
                            <a id="twitter"
                               href="https://twitter.com/intent/tweet?hashtags=antvel&text={{ urlencode($product->name) }}&url={{ Request::url() }}&via=_antvel"
                               class="btn btn-success btn-sm full-width">
